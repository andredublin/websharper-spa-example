...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,UI,Next,Doc,List,SPAExample,Client,Concurrency,Data,WBRuntime,WorldBankRuntime,Seq,Charting,Chart,Arrays,Renderers,ChartJs,String,AttrProxy,T,Collections,ResizeArray,ResizeArrayProxy,JavaScript,JSModule,Random,Pervasives,Color;
 Runtime.Define(Global,{
  SPAExample:{
   Client:{
    ...
    {
     var arg20;
     arg20=List.ofArray([Doc.TextNode("Teritary school enrollment (% gross)")]);
     return Doc.RunById("main",Doc.Concat(List.ofArray([Doc.Element("h2",[],arg20),Doc.Async(Client.chart()),Client.legend()])));
    }),
    chart:Runtime.Field(function()
    {
     var cfg;
     cfg={
      pointDot:false,
     ...
     return Concurrency.Delay(function()
     {
      var mapping,source,arg00,x;
      mapping=function(c)
      {
       var _this;
       _this=WorldBankRuntime.GetIndicators(c);
       return WorldBankRuntime.AsyncGetIndicator(_this,"SE.ENR.TERT.FM.ZS");
      };
      source=Client.countries();
      arg00=Seq.map(mapping,source);
      x=Concurrency.Parallel(arg00);
      return Concurrency.Bind(x,function(_arg1)
      {
       var mapping1,mapping2,array2,array,arg001,x1;
       mapping1=function(i)
       {
        return Client.mkData(i);
       };
       mapping2=function(tupledArg)
       {
        ...
        c=tupledArg[0];
        e=tupledArg[1];
        return Chart.Line(e).__WithStrokeColor(c).__WithPointColor(c);
       };
       array2=Arrays.map(mapping1,_arg1);
       array=Arrays.zip(Client.colors(),array2);
       arg001=Arrays.map(mapping2,array);
       x1=Chart.Combine(arg001);
       return Concurrency.Return(ChartJs.Render2(x1,{
        $:1,
        $0:{
         $:0,
         $0:600,
    ...
    colors:Runtime.Field(function()
    {
     ...
     {
      return(Client.randomColor())(null);
     },Client.countries());
    }),
    countries:Runtime.Field(function()
    {
     ...
     _this2=Client.data();
     _this3=Client.data();
     return[WorldBankRuntime.GetCountry(_this,"AUT","Austria"),WorldBankRuntime.GetCountry(_this1,"HUN","Hungary"),WorldBankRuntime.GetCountry(_this2,"GBR","United Kingdom"),WorldBankRuntime.GetCountry(_this3,"USA","United States")];
    }),
    ...
    legend:Runtime.Field(function()
    {
     var array2,x,mapping,x1;
     array2=Client.colors();
     x=Arrays.zip(Client.countries(),array2);
     mapping=function(tupledArg)
     {
      var c,color,arg20,arg00,arg201;
      c=tupledArg[0];
      color=tupledArg[1];
      arg00="width:200px;height:20px;margin-right:10px;display:inline-block;background-color:"+String(color);
      arg201=List.ofArray([Doc.TextNode(c.Name)]);
 ...
 Runtime.OnInit(function()
 {
  ...
  Doc=Runtime.Safe(Next.Doc);
  List=Runtime.Safe(Global.WebSharper.List);
  SPAExample=Runtime.Safe(Global.SPAExample);
  Client=Runtime.Safe(SPAExample.Client);
  Concurrency=Runtime.Safe(Global.WebSharper.Concurrency);
  Data=Runtime.Safe(Global.WebSharper.Data);
  WBRuntime=Runtime.Safe(Data.WBRuntime);
  ...
  Random=Runtime.Safe(Global.WebSharper.Random);
  Pervasives=Runtime.Safe(Charting.Pervasives);
  return Color=Runtime.Safe(Pervasives.Color);
 });
 Runtime.OnLoad(function()
 {
  Client.randomColor();
